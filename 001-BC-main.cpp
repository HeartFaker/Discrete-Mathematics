...
int main() 
{
	...
	cout << formula << endl;
	int *m;
	m = (int*)malloc(sizeof(int)*pow2(proposition_set.size()));   //该数组依次存放命题公式的各行(0或1)计算结果的值
...
int findProposition(Map_ic pSet, char p) //返回-1，表示该命题变项尚未被遍历过，可计数；否则说明该命题变项已被遍历过，则不重复计数。同时，还可以返回指定命题变项的下标
{
...
int calculate(string formula, Map_ic pSet, Map_ii value) //该函数返回给定0或1组合的运算结果
{
	stack<char> opter;
	stack<int> pvalue;
	opter.push('#');
...
void check(stack<int> &value, stack<char> &opter) //该函数返回两个命题变项(取值0或1)的各种运算结果(0或1)
{
	...
	default:
		cout << "Error Happen! Please Retry" << endl;
		break;
	}
}